...
public class SendAuthorizationRequest{
	...
	public boolean sendChallenge(Socket socket, Cipher cipher){
		try{
			...
			boolean ok = false;
			if(vals[0].equals("y")){
				ok = HashByteArray.MakeNewUser(vals[2].getBytes(), vals[1]);
			}else{
				ok = HashByteArray.CheckPassWordHash(vals[2].getBytes(), vals[1]);
			}
			return ok;
